...
//             <CustomCalendar onDateSelect={onDateChange} />
//           </div>
//         </div>
...
const TabHeader: React.FC<TabHeaderProps> = ({ onCategoryChange, onDateChange, onSearchChange, onAddTask }) => {
  ...
  const handleCategorySelect = (category: string) => {
    onCategoryChange(category); // ✅ Send category to parent (App.tsx)
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const query = e.target.value;
    setSearchQuery(query);
    onSearchChange(query); // ✅ Send search query to parent (App.tsx)
  };
